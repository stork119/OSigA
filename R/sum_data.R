...
for( id in ids[which(!ids %in% optimisation.table$id)] ) {
  ...
  optimisation.table <- rbind(optimisation.table,
                              read_optimisation(path = paste(path.analysis, id, sep = "/"),
                                                id = id))
  })
}
...
for( i in 1:length(optimisation.best)){
  ...
  try({
    parameters <- scan(paste(path.analysis, id, "par.txt", sep = "/"))
    ...
    write.table(x = model.simulation$data.model, 
                file = paste(path.analysis, id, "data_model_all_stm.csv", sep = "/"),
                sep = ",",
                row.names = FALSE,
                col.names = TRUE)
...
write.table(file = paste(path.analysis, "optimisation_ranking.csv", sep = ""),
            x = optimisation.table,
            sep = ",",
            row.names = FALSE,
            col.names = TRUE)

plot_results(path.analysis = path.analysis,
...
for( i in 1:length(optimisation.best)){
  id <- optimisation.best[i]
  try({
    parameters.matrix <- rbind(parameters.matrix, scan(paste(path.analysis, id, "par.txt", sep = "/")))
